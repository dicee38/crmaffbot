...
9. **Mini App: домен обязательно зарегистрировать в @BotFather**, иначе `Telegram.WebApp.initData` всегда будет пустым (сама страница при этом открывается нормально — Telegram отдаёт `platform`/`version`/`theme`, но не подписанные данные пользователя, пока домен не подтверждён). Без этого шага никакой код на нашей стороне initData не получит.
   - @BotFather → `/mybots` → выбрать бота → **Bot Settings** → **Menu Button** → **Configure Menu Button** → указать `MINIAPP_URL` (например `https://<backend-domain>.up.railway.app/miniapp/`) и текст кнопки.
   - Это же действие включает и системную кнопку меню рядом с полем ввода — работает независимо от кнопок в самом боте.
